...
.claude/
├── commands/
│   ├── enrichment-maker.md        # Slash command definition
│   └── migrate-v1-v2.md
│
└── agents/                         # (future: if we add custom agents)
...
├── component-adapter.ts            # Format conversion
├── enrichment-filter.ts            # Filtering logic
├── smart-enrichment-selector.ts    # Intent detection
└── index.ts                        # Exports
```
...
```
src/types/
├── index.ts                   # Main types (Component, Prop, Event)
├── enrichment-options.ts      # Enrichment-specific types
└── mcp-protocol.ts            # MCP protocol types (if needed)
```
...
```
src/services/
├── enrichment-filter.ts
└── __tests__/
    └── enrichment-filter.test.ts
```
...
**Scope (optional):**
- `filter`, `adapter`, `tools`, `enrichment`, `docs`, `conventions`
...
# Breaking change
feat(tools)!: change get-component API
...
./scripts/validate-commit-msg.sh "your commit message"
```

**Git Template:**
```bash
# Use template for manual commits
